...
Default model: AITeamVN/Vietnamese_Reranker (specialized for Vietnamese).
...
class RerankerService:
        ...
        if self._model is None:
            with self._lock:
                if self._model is None:
                    ...
                    # use_fast=False works around the tokenization error
                    # with AITeamVN/Vietnamese_Reranker in transformers
    ...
    def rerank(
        ...
        # Normalize Unicode to NFC for Vietnamese handling
        norm_query = unicodedata.normalize('NFC', query)
        norm_docs = [unicodedata.normalize('NFC', doc) for doc in documents]

        # Build (query, document) pairs for the cross-encoder
        pairs = [(norm_query, doc) for doc in norm_docs]
