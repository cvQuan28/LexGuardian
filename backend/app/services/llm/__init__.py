...
def get_llm_provider() -> LLMProvider:
    ...
    if provider in ("ollama", "ollama-cloud", "ollama_cloud"):
    ...
    raise ValueError(f"Unknown LLM_PROVIDER: {provider!r}. Supported: gemini, ollama, ollama-cloud")
...
def get_embedding_provider() -> EmbeddingProvider:
    ...
    if provider in ("ollama", "ollama-cloud", "ollama_cloud", "ollama-local", "ollama_local"):
        from app.services.llm.ollama import OllamaEmbeddingProvider

        # Nếu user yêu cầu local rẽ nhánh ngay cả khi đang dùng cloud cho LLM
        is_local = provider in ("ollama-local", "ollama_local")
    ...
    raise ValueError(
        ...
        "Supported: gemini, ollama, ollama-cloud, sentence_transformers"
    )
